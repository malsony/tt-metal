...
inline void _init_sdpa_reduce_max_row_8x32_replay_buffers_() {
    // ***********************************************************
...
inline void _init_sdpa_reduce_sum_row_8x32_replay_buffers_() {
    // ***********************************************************
    // Record replay buffer
    // LREG0 will contain the first 4 rows, LREG2 will contain the second 4 rows
    load_replay_buf<NoExec>(sdpa_reduce_row::replay_start, 16, [] {
...
inline void _calculate_sdpa_reduce_row_8x32_(uint src_index) {
    static_assert(
        reduce_dim == ReduceDim::REDUCE_ROW, "Only row reduction (ReduceDim::REDUCE_ROW) is currently supported");
    static_assert(
        pool_type == PoolType::MAX || pool_type == PoolType::SUM,
        "Unsupported pool type. Supported pool types: MAX, SUM");
