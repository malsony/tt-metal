...
// Flatten PadValue to float for fill_implicit_tile_padding. uint32 + FLOAT32: bitcast; else static_cast
// from integer (not a bit pattern). Some integer pad values inexact in float.
    ...
    if (memory_config.memory_layout() == TensorMemoryLayout::WIDTH_SHARDED ||
        ...
        if (tensor.dtype() == DataType::BFLOAT8_B) {
            ...
            auto output_tensor_3d = ttnn::prim::reshape_view(
                ...
                sub_core_grid);
            // Fill implicit tile padding while still interleaved + BFLOAT16. Tensor is rank-3,
            // so fill_implicit_tile_padding stays on its non-recursive path.
